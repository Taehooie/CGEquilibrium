...
lambda_cost = tf.zeros((len(od_path_inc),1), tf.float32)        # 6
lambda_flow_bal = tf.zeros((len(od_path_inc),1), tf.float32)        # 6
lambda_positve = tf.zeros((len(path_link_inc_n) + len(path_link_inc),1), tf.float32)        # 9
...
def calculateCoreVars(access_cost_, path_flow_):
    ...
    auto_volume = od_volume * (tf.exp(-theta*access_cost_) + tf.cast(tf.exp(-theta*transit_cost), tf.float32))
    path_flow_n = auto_volume - tf.matmul(tf.cast(od_path_inc,  tf.float32), path_flow_)
